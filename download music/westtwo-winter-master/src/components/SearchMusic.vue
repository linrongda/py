<template>
  ...
  <div class="search-box">
    ...
    <div v-show="activeIndex === '0'">
      <el-input v-model="searchText" size="large" placeholder="请输入要搜索的歌曲和歌手" class="input-with-select">
        <template #append>
          <el-button :icon="Search" @click="refreshsearch"></el-button>
        </template>
      </el-input>
      ...
      <div class="table-bottom"> 
        <el-pagination v-model:current-page="pagination0.currentPage" v-model:page-count="pagination0.pageCount"
    ...
    <div v-show="activeIndex === '1'">
      <div class="table-top">
        ...
        <el-button type="danger" :disabled="historyTableData.length === 0"
          @click="deleteHistoryRequest(1)">批量删除</el-button>
      </div>
      <el-table :data="historyTableData" stripe style="width: 100%" ref="historyTableRef"
      ...
      <div class="table-bottom">
        <el-pagination v-model:current-page="pagination.currentPage" v-model:page-count="pagination.pageCount"
          background layout="prev, pager, next" />
      </div>
    </div>
  </div>
...
const tableRef = ref()
const tableData = ref([])
const pagination0 = reactive({
  currentPage: 1,
  pageCount: 1
})
//删除原来的搜索结果展示，借鉴历史记录的代码更改为分页展示
watch(() => pagination0.currentPage, (value, oldValue, onCleanup) => {
  console.log(value, oldValue)
  refreshsearch(value)
  tableData.value = null; // 清空 tableData.value 的数据
  onCleanup(() => {
    tableData.value = null; // 在 watch 停止监听时，再次清空 tableData.value 的数据
  });
})
//对了，不知道为什么下面的page: pagination0.currentPage改为page: page的时候会报错
const refreshsearch = (page = 1) => {
  console.log(page)
  const data = {
    text: searchText.value,
    page: pagination0.currentPage
  }
  getSearchList(data).then(res => {
    console.log(res.data.data.list)
    tableData.value = res.data.data.list
    if (res.data.data.count) {
      pagination0.pageCount = res.data.data.count
    }
  })
}
...
const downloadRequest = (row) => {
  downloadMusic(row.rid).then(res => {
    ...
    downloadElement.download = `${row.name}-${row.artist}.mp3`; //下载后文件名
